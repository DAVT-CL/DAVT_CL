...
def incremental_train_and_eval_DCE_MR_LF_TDE(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
            ...
            iteration, start_iteration, last_iter, nb_cl,\
            lamda, \
            dist, K, lw_mr, \
    ...
    for epoch in range(epochs):
        ...
        if iteration == start_iteration:
            with tqdm(total=len(trainloader), position=0) as bar:
                bar.set_description(
                    "[from {} to {} epoch: {} in {}]".format(last_iter * nb_cl, (iteration + 1) * nb_cl, epoch, epochs))
                for batch_idx, (inputs, targets) in enumerate(trainloader):
                    ...
                    bar.set_postfix({'loss': '{:.4f}'.format(train_loss/(batch_idx+1))})
                    bar.update(1)
            ...
            if dataset=='cifar':
                input_X_train, old_feat_X_train, targets_X_train, inter_atten = compute_features_FeatCluster(
                    ref_feature_model, trainloader, num_samples, num_features, return_data=True, device=device)
            else:
                input_X_train, old_feat_X_train, targets_X_train, inter_atten = compute_features_FeatCluster_imagenet(
                    ref_feature_model, trainloader, num_samples, num_features, return_data=True, device=device)
            ...
            start_idx = 0
            batch_idx = 0
            with tqdm(total=int(math.ceil(num_samples/batch_size)), position=0) as bar:
                bar.set_description(
                    "[from {} to {} epoch: {} in {}]".format(last_iter * nb_cl, (iteration + 1) * nb_cl, epoch, epochs))
                while start_idx < num_samples:
                    if start_idx + batch_size < num_samples:
                        inputs = input_X_train[start_idx:start_idx + batch_size, :, :, :]
                        old_inter_atten = inter_atten[start_idx:start_idx + batch_size, :, :, :]
                        ...
                        old_inter_atten = inter_atten[start_idx:, :, :, :]
                    ...
                    new_inter_atten = tg_model.inter_atten
                    ...
                    loss4 = nn.CosineEmbeddingLoss()(pool(new_inter_atten), pool(old_inter_atten.to(device)), \
                                                     torch.ones(new_inter_atten.shape[0]).to(device)) * alpha
        ...
        total = 0
        with torch.no_grad():
            with tqdm(total=len(testloader), position=0) as eval_bar:
                eval_bar.set_description("[test]")
