...
def compute_accuracy_TDE(tg_model, tg_feature_model, class_means, evalloader, causal_embed, iteration=0, scale=None, print_info=True, device=None, num_crnt_class=None):
    ...
    result = []
    if print_info:
        print("  top 1 accuracy DVT           :\t\t{:.2f} %".format(100.*correct_cnn/total))
        print("  top 1 accuracy iCaRL          :\t\t{:.2f} %".format(100.*correct_icarl/total))
        print("  top 1 accuracy NCM            :\t\t{:.2f} %".format(100.*correct_ncm/total))
        result.append(round(100.*correct_cnn/total, 2))
        result.append(round(100.*correct_icarl/total, 2))
        result.append(round(100.*correct_ncm/total, 2))

        if iteration > 0:
            print("  top 1 accuracy DVT-TDE          :\t\t{:.2f} %".format(100.*correct_TDE/total))
            result.append(round(100.*correct_TDE/total, 2))
    return torch.tensor(100*np.array(all_accu)), np.array(result)
...
def compute_accuracy_finetune_alpha_TDE(nb_inc, nb_cl_fg, nb_cl, tg_model, tg_feature_model, class_means, 
    ...
    if print_info:
        ...
        print("  top 1 accuracy NCM            :\t\t{:.2f} %".format(100.*correct_ncm/total))
        if iteration > 0:
            print("  top 1 accuracy DVT-CBF-TDE          :\t\t{:.2f} %".format(100.*correct_cbf_TDE/total))
