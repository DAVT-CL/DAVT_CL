...
parser.add_argument('--dataset', default='seed_1999_tini_imagenet', type=str)
parser.add_argument('--random_seed', default=1999, type=int, \
    help='random seed')
parser.add_argument('--datadir', default='/home/dataSet/tiny_imagenet_200/', type=str)
...
parser.add_argument('--nb_protos', default=1, type=int,
...
parser.add_argument('--ckp_prefix', default=os.path.basename(sys.argv[0])[:-3], type=str, \
    help='checkpoint_tini_step_10_1 prefix')
...
parser.add_argument('--resume', action='store_true', \
    help='resume from checkpoint_tini_step_10_1')
...
for iteration_total in range(args.nb_runs):
    ...
    order_name = "./checkpoint_tini_step_10_1/seed_{}_{}_order_run_{}.pkl".format(args.random_seed, args.dataset, iteration_total)
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        if iteration == start_iter:
            ...
            tg_model = DVT(
                ...
                mlp_mult=1,
                cnnbackbone="resnet_cifar100",   # 'resnet_cifar100', 'ResNet18Pre' 'ResNet164'
        ...
        ckp_path = './checkpoint_tini_step_10_1/{}'.format(args.ckp_prefix)
        if not os.path.exists(ckp_path):
            os.mkdir(ckp_path)
        # if iteration == start_iter:
        #     ckp_name = "/home/DAVT_CIL/imagenet-class-incremental/checkpoint/class_incremental_cosine_imagenet_nb_cl_fg_50_nb_cl_10_nb_protos_20_top_1/run_0_iteration_4_model.pth"
        # else:
        ...
        if args.resume and os.path.exists(ckp_name):
            print("###############################")
            print("Loading models from checkpoint_tini_step_10_1")
            if iteration > start_iter:
                ...
                if args.TDE:
                    if args.DCE:
                        ...
                        tg_model, causal_embed = incremental_train_and_eval_DCE_MR_LF_TDE(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                            ...
                            train_batch_size, num_samples, dataset='imagenet', top_k=args.top_k, device=device)
                ...
                if args.less_forget:
                    ...
                    if args.mimic_score:
                        ...
                        tg_model = incremental_train_and_eval(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                            ...
                            iteration, start_iter,
                            args.T, args.beta)
            if not os.path.isdir('checkpoint_tini_step_10_1'):
                os.mkdir('checkpoint_tini_step_10_1')
        ...
        cm_name = './checkpoint_tini_step_10_1/run_{}_iteration_{}_confusion_matrix.pth'.format(iteration_total, iteration)
