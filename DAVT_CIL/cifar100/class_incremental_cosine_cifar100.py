...
parser.add_argument('--ckp_prefix', default=os.path.basename(sys.argv[0])[:-3], type=str, \
    help='checkpoint_attn1_step_5_2000 prefix')
...
parser.add_argument('--resume', action='store_true', \
    help='resume from checkpoint_attn1_step_5_2000')
...
########################################
# improved class incremental learning of TDE and DCE
parser.add_argument('--TDE', default=True, action='store_true', \
    help='moving average of feature')
parser.add_argument('--DCE', default=True, action='store_true', \
    help='train with DCE')
...
args.ckp_prefix        = '{}_nb_cl_fg_{}_nb_cl_{}_nb_protos_{}_cos_3'.format(args.ckp_prefix, args.nb_cl_fg, args.nb_cl, args.nb_protos)
...
trainset = torchvision.datasets.CIFAR100(root='/home/dataSet/', train=True,
                                        download=True, transform=transform_train)
testset = torchvision.datasets.CIFAR100(root='/home/dataSet/', train=False,
                                       download=True, transform=transform_test)
evalset = torchvision.datasets.CIFAR100(root='/home/dataSet/', train=False,
...
for iteration_total in range(args.nb_runs):
    ...
    # order_name = "./checkpoint_attn1_step_5_2000/seed_{}_{}_order_run_{}.pkl".format(args.random_seed, args.dataset, iteration_total)
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        ##############################################################
        ckp_path = './checkpoint_attn1_step_5_2000/{}'.format(args.ckp_prefix)
        if not os.path.exists(ckp_path):
            os.mkdir(ckp_path)
        # if iteration == start_iter:
        #     ckp_name = "/home/DAVT_CIL/cifar100-class-incremental/checkpoint_attn1_step_5_2000/class_incremental_cosine_cifar100_nb_cl_fg_50_nb_cl_10_nb_protos_20_cos_3_top_10/run_0_iteration_4_model.pth"
        # else:
        ckp_name = ckp_path+'/run_{}_iteration_{}_model.pth'.format(iteration_total, iteration)
        print('ckp_name', ckp_name)
        if args.resume and os.path.exists(ckp_name):
            print("###############################")
            print("Loading models from checkpoint_attn1_step_5_2000")
                ...
                if args.TDE:
                    if args.DCE:
                        ...
                        tg_model, causal_embed = incremental_train_and_eval_DCE_MR_LF_TDE(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                            ...
                            iteration, start_iter, last_iter, args.nb_cl,\
                            cur_lamda, \
                            args.dist, args.K, args.lw_mr,\
                            train_batch_size, num_samples, dataset='cifar', top_k=args.top_k, device=device)
                        np.save(ckp_path+'/causal_embed_run_{}_iteration_{}.npy'.format(iteration_total, iteration), causal_embed.detach().cpu())
                    else:
                        print("incremental_train_and_eval_MR_LF_TDE")
                        tg_model, causal_embed = incremental_train_and_eval_MR_LF_TDE(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                            trainloader, testloader, \
                            iteration, start_iter, last_iter, args.nb_cl, \
                            cur_lamda, \
                            args.dist, args.K, args.lw_mr) 
                ...
                if args.less_forget:
                    ...
                    if args.mimic_score:
                        ...
                        tg_model = incremental_train_and_eval(args.epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
                            ...
                            iteration, start_iter,
                            args.T, args.beta) 
            if not os.path.isdir('checkpoint_attn1_step_5_2000'):
                os.mkdir('checkpoint_attn1_step_5_2000')
        ...
        class_means_TDE = np.zeros((num_features, args.num_classes, 2))
        ...
        if iteration > start_iter:
            ...
            for i in range(iteration-start_iter+1):
                ...
                print('Computing accuracy on the previous classes step_{}...'.format(i))
                evalset.data = X_valid_cumuls[i].astype('uint8')
                evalset.targets = map_Y_valid_prev
                evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                        shuffle=False, num_workers=2)
                if args.TDE:
                    ...
                    ori_acc, result = compute_accuracy_TDE(tg_model, tg_feature_model, current_means,
                        evalloader, causal_embed, iteration=iteration-start_iter, print_info=True, device=device)
                    resutl_steps.append(result)
                else:
                    ori_acc = compute_accuracy(tg_model, tg_feature_model, current_means, evalloader, device=device)
        ...
        if iteration>start_iter and args.TDE:  
            ...
            causal_embed = torch.from_numpy(causal_embed).to(device)
            causal_embed = causal_embed/(torch.norm(causal_embed, 2, 1, keepdim=True))
            cumul_acc, result = compute_accuracy_TDE(tg_model, tg_feature_model, current_means,
                 evalloader, causal_embed, iteration=iteration-start_iter, device=device)
            resutl_steps.append(result)
            final_result = np.stack(resutl_steps, axis=0).T
            print("final_result:", final_result)
            pd.DataFrame(final_result).to_csv(ckp_path+'/step_{}.csv'.format(iteration))
        ...
        cm = compute_confusion_matrix(tg_model, tg_feature_model, current_means, evalloader, device=device)
